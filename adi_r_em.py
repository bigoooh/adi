...
from keras.layers import Embedding,SimpleRNN,Masking,Input,Conv1D,Conv2D,Conv3D, BatchNormalization, MaxPooling1D,MaxPooling2D,AveragePooling2D,MaxPooling3D, Dense, Input, Dropout, Flatten,GRU,TimeDistributed,Bidirectional,LSTM,Input,concatenate,Activation,average
...
def create_model(conv_units=128, lr=1e-4,drop=0.2,conv_layers=3):
    ...
    em_input=Input(shape=(600,))
   
    em_drp=Dropout(0.7)(em_input)
    em_output=Dense(5,activation='softmax')(em_drp)

    

    avg=average([ch_ouput,em_output])
    model=Model(inputs=[ch_input,em_input],outputs=avg)
...
X_train,y_train,X_dev,y_dev=get_data('train_dev/train.words','train_dev/dev.words')
print('Tokenization')
...
em=np.loadtxt('train_dev/train.embedding',dtype='str')
X_train_em=np.array(em[:,:-1],dtype='float')
#y_train_em=np.array(em[:,-1],dtype='str')
em=np.loadtxt('train_dev/dev.embedding',dtype='str')
X_dev_em=np.array(em[:,:-1],dtype='float')
#y_dev_em=np.array(em[:,-1],dtype='str')
X_all_em=np.concatenate((X_train_em,X_dev_em))
...
for t,v in kf.split(X_all):
    ...
    model = create_model(conv_layers=3,conv_units=256,lr=5e-4,drop=0.3)
    checkpoint_dir='k_10_%d'%(i)
    ...
    early_stoppig=keras.callbacks.EarlyStopping(monitor='val_acc', min_delta=0, patience=20, verbose=0, mode='auto')
    reduce_lr=keras.callbacks.ReduceLROnPlateau(monitor='val_acc', factor=0.2, patience=5,  min_lr=5e-6)
    model.fit([X_train,X_train_em], y_train,validation_data=([X_dev,X_dev_em],y_dev),callbacks=[logger,early_stoppig,reduce_lr,check_point],verbose=1,epochs=100,batch_size=128)
